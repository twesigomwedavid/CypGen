...
process filter_bam {
   ...
   output:
      set val(name), file("${name}_filtered.bam") into aln_ch

   script:
    """
       samtools view ${bam.get(0)} 22:42518000-42555000 -o ${name}_filtered.bam
...
//   maxForks 5
   ...
   input:
      set val(name), file(bam_region) from aln_ch
   ...
   script:
    """
        samtools index ${bam_region} 
   	graphtyper genotype $hg19 --sam=${bam_region} --region=22:42522300-42528400 --output=${name}_var --prior_vcf=$in_vars -a --no_cleanup       
    """
...
//   maxForks 5
   ...
   script:
    ...
    """
      ...
      python /home/david/asterigraph/workflows/bin/test4.py ${db_core_snvs} ${name}_vars/${name}_core_snvs.dip ${name}_vars/${name}_full.dip ${name}_vars/${name}_gt.dip ${db_tiebreak} > ${name}_snv_def.alleles
