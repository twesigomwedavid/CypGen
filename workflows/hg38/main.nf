...
hg38 = file(params.hg38_ref, type: 'file')
...
cypgen_mode = file(params.call_script, type: 'file')
...
//   maxForks 5
   ...
   script:
    """
   	graphtyper genotype $hg38 --sam=${bam.get(0)} --region=22:42126000-42146000 --output=${name}_var_1 --prior_vcf=${in_vars}
	graphtyper genotype $hg38 --sam=${bam.get(0)} --region=22:42126000-42146000 --output=${name}_var_2
	bcftools isec -p ${name}_var -Oz ${name}_var_1/22/042126000-042146000.vcf.gz ${name}_var_2/22/042126000-042146000.vcf.gz 	
	bcftools concat -a -D -r 22:42126300-42132400 ${name}_var/0000.vcf.gz ${name}_var/0001.vcf.gz ${name}_var/0002.vcf.gz -Oz -o ${name}_var/${name}_042126300-042132400.vcf.gz
	tabix ${name}_var/${name}_042126300-042132400.vcf.gz  
	graphtyper genotype_sv $hg19 --sam=${bam.get(0)} --region=22:42126000-42144000 --output=${name}_sv_del ${coded_star_5}
	graphtyper genotype_sv $hg19 --sam=${bam.get(0)} --region=22:42126000-42144000 --output=${name}_sv_dup ${coded_full_gendup}
...
//   maxForks 5
   ...
   script:
    ...
    """
      bcftools norm -m - ${name}_vars/${name}_042126300-042132400.vcf.gz | bcftools view -e 'GT="1/0"' | bcftools view -e 'GT="0/0"' | bgzip -c > ${name}_vars/${name}_all_norm.vcf.gz
      ...
      bcftools query -f'%ID\t%ALT\t[\t%GT\t%DP]\t%INFO/ABHet\t%INFO/ABHom\n' ${name}_gene_del/22/042126000-042144000.vcf.gz > ${name}_gene_del/${name}_gene_del_summary.txt
      bcftools norm -m - ${name}_gene_dup/22/042126000-042144000.vcf.gz | bcftools view -e 'GT="1/0"' | bcftools view -e 'GT="0/0"' | bgzip -c > ${name}_gene_dup/22/${name}_dup_norm.vcf.gz
      ...
      python ${cypgen_mode} ${db_core_snvs} ${name}_vars/${name}_core_snvs.dip ${name}_vars/${name}_full.dip ${name}_vars/${name}_gt.dip ${db_tiebreak} ${name}_gene_del/${name}_gene_del_summary.txt ${name}_gene_dup/${name}_gene_dup_summary.txt ${name}_2d6_dp ${db_haps} > ${name}_snv_def.alleles
